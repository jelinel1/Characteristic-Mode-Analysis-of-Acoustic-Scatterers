...
template <typename T>
__device__ void solveZE(T * outReal, T * outImag, const T * weights,
...
//   size_t const blockPos = threadIdx.y*blockDim.x + threadIdx.x;
  ...
  positiveXYDivided(-wavelength*dist, weight, resultReal, resultImag, dist, wavelength);
  
  dist = computeDist(&radius1[(q+quad)*3], &radius2[p*3]);
  negativeXYDivided(-wavelength*dist, weight, resultReal, resultImag, dist, wavelength);
...
__global__ void solveZEf(float * outReal, float * outImag, const float * weights,
        const float * radiusVectors, const float wavelength,
        const int quad, const float* edgeLengths, const float scale, const int xMin,
        const int xMax, const int yMin, const int yMax, const int outputSize) {
  solveZE(outReal, outImag, weights, radiusVectors, wavelength,
          quad, edgeLengths, scale, xMin, xMax, yMin, yMax, outputSize);
}

__global__ void solveZEd(double * outReal, double * outImag, const double * weights,
        const double * radiusVectors, const double wavelength,
