...
template <typename T>
__device__ void solveTM(T * outReal, T * outImag, const T * weights,
        const T * radiusVectors, const T * basisVectors,
        const T wavelength, const int quad, const T* edgeLengths,
        const T scale, const int xMin, const int xMax, const int yMin,
        const int yMax, const int outputSize) {
...
//   size_t const blockPos = threadIdx.y*blockDim.x + threadIdx.x;
  ...
  positiveXY(-wavelength*dist, weight*basisDotProduct, resultReal, resultImag);
  
  dist = computeDist(&radius1[(q+quad)*3], &radius2[p*3]);
  basisDotProduct = dotProduct3(&basis1[(q+quad)*3], &basis2[p*3]);
  negativeXY(-wavelength*dist, weight*basisDotProduct, resultReal, resultImag);
...
__global__ void solveTMf(float * outReal, float * outImag, const float * weights,
  ...
  solveTM(outReal, outImag, weights, radiusVectors, basisVectors, wavelength,
          quad, edgeLengths, scale, xMin, xMax, yMin, yMax, outputSize);
}

__global__ void solveTMd(double * outReal, double * outImag, const double * weights,
