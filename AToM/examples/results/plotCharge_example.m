%% plotCharge_example
% Example of charge visualisation
%
...
results.plotCharge(mesh, 'basisFcns', basisFcns, 'iVec', iVec(:, 5));

%% basic usage with output parameters
% output: references to all graphical objects in figure
handles = results.plotCharge(mesh, 'basisFcns', basisFcns, 'iVec', ...
    iVec(:, 5));
...
handles2 = results.plotCharge(mesh, 'basisFcns', basisFcns, ...
    'iVec', iVec(:, 5), 'options', chargeOptions);

%% select part of shown current
% possible input: {'Im', 'Re', 'Abs'}
handles3 = results.plotCharge(mesh, 'basisFcns', basisFcns, ...
    'iVec', iVec(:, 5), 'part', 'Re');

%% calculate charge and plot previously calculated charge
divJ = results.calculateCharge(mesh, basisFcns, iVec);

handles = results.plotCharge(mesh, 'part', 'Re', 'divJ', divJ(:, 5));